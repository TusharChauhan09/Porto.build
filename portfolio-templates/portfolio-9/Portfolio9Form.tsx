...
export default function Portfolio9Form({ initialData, onChange }: Portfolio9FormProps) {
    ...
    <div className="space-y-8 p-4">
      ...
      <fieldset className="space-y-3">
        ...
        <label className="block">
          ...
          <p className="text-xs text-muted-foreground">Displayed in italic serif under "The Philosophy" heading in the hero.</p>
          <textarea
            value={data.bio}
            onChange={(e) => update({ bio: e.target.value })}
            rows={3}
            className="mt-1 w-full rounded border border-input bg-background px-3 py-2 text-sm"
            placeholder="Defining the intersection of avant-garde digital solutions and heritage luxury branding..."
          />
        </label>

        <label className="block">
          <span className="text-sm font-medium">Profile Image URL *</span>
          <p className="text-xs text-muted-foreground">Displayed as a large circular image with gold ring and glow effect.</p>
          <input
            type="text"
            value={data.image}
      ...
      <fieldset className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">Skills appear as elegant list items with level labels (Foundation, Proficient, Advanced, Mastery).</p>
          ...
          <div key={i} className="space-y-2 rounded border border-input p-3">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 min-w-0 rounded border border-input bg-background px-3 py-2 text-sm"
                placeholder="Visual Identity Systems"
              />
              <select
      ...
      <fieldset className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">Projects display as tall cards with zoom-on-hover images and "View Case Study" overlay. Tags appear as gold-bordered pills.</p>
        {data.projects.map((proj, i) => (
          <div key={i} className="space-y-2 rounded border border-input p-3">
            <input
              ...
              placeholder="The Sovereign Collection"
            />
            <textarea
            ...
            <input
              ...
              className="w-full rounded border border-input bg-background px-3 py-2 text-sm"
              placeholder="Tags (comma separated): Web3, UX, Branding..."
            />
            <input
              type="text"
              value={proj.image ?? ""}
              ...
              className="w-full rounded border border-input bg-background px-3 py-2 text-sm"
              placeholder="Project Image URL (recommended — tall 4:5 ratio)"
            />
      ...
      <fieldset className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">Displayed as an elegant timeline with gold dots and serif role titles.</p>
        ...
        {data.education.map((edu, i) => (
          <div key={i} className="space-y-2 rounded border border-input p-3">
            <input
              ...
              className="w-full rounded border border-input bg-background px-3 py-2 text-sm"
              placeholder="Royal College of Art, London"
            />
            <div className="flex gap-2">
              <input
                type="text"
        ...
        {data.certifications.map((cert, i) => (
          <div key={i} className="space-y-2 rounded border border-input p-3">
            <input
              ...
              className="w-full rounded border border-input bg-background px-3 py-2 text-sm"
              placeholder="Master of Design Systems"
            />
            <div className="flex gap-2">
              <input
                type="text"
